...
detect_nixos_adjust() {
  ...
  if hostnamectl | grep -q 'Operating System: NixOS'; then
    ...
    grep -Fqx '    hl.exec_cmd(os.getenv("HOME") .. "/.config/hypr/scripts/Polkit-NixOS.sh")' "$OVERLAY_SA" || sed -i '/^end)$/i\    hl.exec_cmd(os.getenv("HOME") .. "/.config/hypr/scripts/Polkit-NixOS.sh")' "$OVERLAY_SA"
